...
在实现路由劫持后，我们现在需要来考虑如果实现处理子应用的逻辑了。

对于一个子应用来说，完整的生命周期势必会经过这三个阶段：

1. 加载子应用资源
2. 挂载子应用
3. 卸载子应用

既然子应用有生命周期，主应用肯定也有，而且也必然是相对应子应用生命周期的。

实际上这就和我们在项目里写的父子组件一样。在初次渲染的时候，父组件肯定会先触发挂载组件前的生命周期，然后子应用触发挂载前及挂载完成的生命周期，接下来父组件在所有子组件完成挂载后触发挂载完成的生命周期，最后如果要卸载组件，肯定也是子组件先触发卸载生命周期，然后父组件才会触发。

因此我们大致可以整理出来主 / 子应用的生命周期。
...
1. `bootstrap`：应用首次加载，常用于配置子应用全局信息
2. `mount`：应用加载时触发，常用于渲染子应用
