...
function checkSelectProp(props, propName) {
  {
    var array = isArray(props[propName]);
...
function renderSuspenseBoundary(request, task, props) {
  ...
  var suspendedFallbackTask = createTask(request, fallback, parentBoundary, innerSegment, fallbackAbortSet, task.legacyContext, task.context, null); // TODO: This should be queued at a separate lower priority queue so that we only work
...
function spawnNewSuspendedTask(request, task, x) {
  ...
  var newTask = createTask(request, task.node, task.blockedBoundary, newSegment, task.abortSet, task.legacyContext, task.context, task.assignID); // We've delegated the assignment.
...
function renderNode(request, task, node) {
  ...
  try {
    ...
    if (typeof x === 'object' && x !== null && typeof x.then === 'function') {
      ...
      switchContext(previousContext); // We assume that we don't need the correct context.
